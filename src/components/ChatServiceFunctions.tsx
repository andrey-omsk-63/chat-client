  ...
  for (let i = 0; i < sistUsers.length; i++) {
    ...
    for (let j = 0; j < archive.length; j++) {
      if (
        ...
        if (archive[j].to !== 'Global' && archive[j].to === paramsName && !archive[j].read) {
          point = '●';
        }
      }
    }
    ...
    resStr.push(
      <Grid key={i} container>
        ...
        <Grid item xs sx={styleChat08}>
          <Box sx={{ color: coler }}>
            {sistUsers[i].user !== paramsName && sistUsers[i].user !== nameKomu && (
...
export const ChatEmojiPicker = (isOpen: boolean, setOpen: Function, onEmojiClick: Function) => {
  return (
    <Box sx={styleChat06}>
