...
const Chat = (props: { ws: WebSocket; nik: any }) => {
  ...
  React.useEffect(() => {
    socket.on("message", (event: any) => {
      console.log("event.data:", event.data, oldRoom);
      setTimeout(() => {
        //let tekRoom = event.data.user.room;
        //if (!tekRoom) tekRoom = oldRoom;
        let mask = {
          from: event.data.user.name,
          to: event.data.user.to,
  ...
  const handleSubmit = () => {
    console.log("handleSubmit", message, params, oldRoom);
    if (!message) return;
      ...
      resStr.push(
        <Grid key={i} item container xs={12}>
          <Grid item xs={12} sx={styleChat08}>
            ...
            {params.name === sistUsers[i].user && <b>{nameer}</b>}
          </Grid>
        </Grid>
      );
    }
