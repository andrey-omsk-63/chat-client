...
const Chat = (props: { ws: WebSocket; Socket: any; nik: any }) => {
  ...
  const SendReguest = React.useCallback(() => {
    ...
    if (debug) {
      ...
      console.log('Position:', tempPosition, maxPosition);
    }
  }, [WS, BeginWork]);
  ...
  }, [WS, PostingArchive, BeginWork, trigger, users, scrollPosition, MessageProcess, SendReguest]);
  //=== РЕЖИМ ОТЛАДКИ ======================================
  React.useEffect(() => {
    if (debug) {
  ...
  const LeftPartChat = () => {
    ...
    // React.useEffect(() => {
    //   if (!scrollPosition) {
    //     console.log('2*ОТПРАВИТЬ ЗАПРОС', scrollPosition);
    //     SendReguest();
    //   }
    // }, [scrollPosition]);
