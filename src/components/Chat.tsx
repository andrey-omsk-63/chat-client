...
const Chat = (props: { ws: WebSocket; Socket: any; nik: any }) => {
  ...
  React.useEffect(() => {
    if (debug) {
      socket.on('message', (event: any) => {
        //console.log("Получение Мessage:", event.data.message);
        let toTo = true;
        ...
        archive.push(mask);
        setTimeout(() => {
          event.data.to === oldRoom && setState((_state) => [..._state, event.data]);
  ...
  const handleClipboard = React.useCallback(
    (evt: any) => {
      ...
      const item = items[0];
      // Получить блок изображения
      const blob: any = item.getAsFile();
      console.log('000££££££', item, blob);

      // Предположим, что у нас есть элемент `img`
      // <img id="preview" />
      const imageEle: any = document.getElementById('preview');
      console.log('111££££££', imageEle);
      ...
      //   console.log('Event',event.target.result); // data url! };
      //   aa = event.target.result
      //   reader.readAsDataURL(blob);
      // };
      setTimeout(() => {
        let aa = JSON.stringify(reader.result).length;
        console.log('444££££££', aa);
        let date = new Date().toISOString();
        let message = JSON.stringify(reader.result).slice(1, 999000);
        //let message = reader.result;
        if (debug) {
          socket.emit('sendMessage', { message, params, date });
          //console.log("MESS", params, message);
        }
      }, 100);
      //======
    },
    ...
    return (
      <>
        ...
        <Box sx={styleChat05}>
          ...
          <img className={styles.preview} id="preview" alt="Pict" />
        </Box>
        {BootomPartChat()}
      </>
    );
