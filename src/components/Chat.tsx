...
const Chat = (props: { ws: WebSocket; Socket: any; nik: any }) => {
  ...
  const handleClipBoard = React.useCallback(
    (evt: any) => {
      ...
      setTimeout(() => {
        let date = new Date().toISOString();
        if (reader.result) {
          let message = reader.result.slice(0, 999000);
          debug && socket.emit('sendMessage', { message, params, date });
          !debug && SendSocketSendMessage(WS, message, params.name, nameKomu);
        }
      }, 100);
  ...
  const LeftPartChat = () => {
    ...
    const OutPutMessage = React.useCallback(() => {
      return <>{state.length && <Messages messages={state} name={pn} basket={stateBasket} />}</>;
    }, [state]);
      ...
      <>
        ...
        <Box sx={styleChat05}>
          <Box ref={scRef} sx={{ overflowX: 'auto', height: '86vh' }}>
            {OutPutMessage()}
            {/* {state.length && <Messages messages={state} name={pn} basket={stateBasket} />} */}
            <div ref={divRef} />
          </Box>
        </Box>
        {BootomPartChat()}
