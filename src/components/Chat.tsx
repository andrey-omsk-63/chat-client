...
const Chat = (props: { ws: WebSocket; Socket: any; nik: any }) => {
  ...
  //                     ? archive[i].message
  //                     : reader.result; // если длина спрессованной картинки < 200байт - косячная картинка
  //                 StateEntry(mess);
  //                 let mask = {
  //                   message: archive[i].message,
  //                   time: archive[i].time,
  ...
  const BeginWork = (arch: any, mode: number) => {
    const DoDoDo = (i: any) => {
      ...
      if (items.message.slice(0, 11) === 'data:image/') {
        ...
        const handleMake = () => {
          if (reader.result !== null) {
            let mess = reader.result.length < 200 ? items.message : reader.result; // если длина спрессованной картинки < 200байт - косячная картинка
    ...
    if (arch) {
      if (arch.messages) {
        // let items = arch.messages;
        // async (items: any) => {
        //   for (let i = 0; i < items.length; i++) {
        //     const result = await DoDoDo(items[i]);
        ...
        //et i = 0;
        // for (; i < arch.messages.length; ) {

        //do {
        //console.log('I=', i, arch.messages[i]);
        // const StateEntry = (MESS: any) => {
        //   let mask = {
        //     from: arch.messages[i].from,
        //     to: arch.messages[i].to,
        ...
        //   blob = MakeNewBlob(arch.messages[i].message);
        //   reader = new FileReader();
        //   compressedFile = null;
        //   handleImageUpload();
        //   console.log('%%%%%%:', compressedFile, reader.result);
        //   const handleMake = () => {
        //     if (reader.result !== null) {
        //       let mess = reader.result.length < 200 ? arch.messages[i].message : reader.result; // если длина спрессованной картинки < 200байт - косячная картинка
        //       console.log('MESS:', mess);
        //       StateEntry(mess);
        //       i++;
        //       console.log('111', i);
        ...
        //   handleMake();
        // } else {
        //   StateEntry(arch.messages[i].message); //текстовое сообщение
        //   i++;
        //   console.log('222', i);
        // }
        //} while (i < arch.messages.length);
      }
    }
    !mode && BeginWorkInRoom('Global', 0, true);
  };
  ...
  const MessageProcess = React.useCallback(
    (data: any) => {
      ...
      if (data.message.slice(0, 11) === 'data:image/') {
        ...
        const handleMake = () => {
          if (reader.result !== null) {
            let mess = reader.result.length < 200 ? data.message : reader.result; // если длина спрессованной картинки < 200байт - косячная картинка
            StateEntry(mess);
            let mask = {
  ...
  const TopPartChat = () => {
    ...
    let redKnop = params.room !== 'Global' ? 'Покинуть комнату' : 'Выйти из чата';
    ...
    return (
      <Box sx={styleChat02}>
        ...
        {!debug && params.room === 'Global' && <>{HeaderChel(users, chel, nameRoom)}</>}
      ...
      setTimeout(() => {
        ...
        if (reader.result) {
          ...
          if (pict.length > 2000000) {
            ...
            let message = debug ? reader.result.slice(0, 999000) : reader.result;
            debug && socket.emit('sendMessage', { message, params, date });
            !debug && SendSocketSendMessage(WS, message, params.name, nameKomu);
          }
        }
      ...
      <>
        ...
        <Box sx={styleChat05}>
          <Box ref={scRef} sx={{ overflowX: 'auto', height: '86vh' }}>
            {state.length !== 0 && (
              <Messages messages={state} name={pn} basket={stateBasket} funcDel={DelRec} />
            )}
            <div ref={divRef} />
          </Box>
        </Box>
